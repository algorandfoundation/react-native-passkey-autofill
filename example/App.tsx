...
import { encodeAddress, toBase64URL, fromBase64Url, generateEd25519KeyPair } from "./utils/crypto";
...
function AppContent() {
  ...
  useEventListener(ReactNativePasskeyAutofill, "onPasskeyAdded", (event) => {
    ...
    alert(
      "Passkey Added: " +
        (event.success ? "Successfully added passkey!" : "Failed to add passkey."),
    );
    bootstrap();
  });
  ...
  useEventListener(ReactNativePasskeyAutofill, "onPasskeyAuthenticated", (event) => {
    ...
    alert(
      "Passkey Authenticated: " +
        (event.success ? "Successfully authenticated!" : "Authentication failed."),
    );
  });
  ...
  const handleGetPasskey = async () => {
    try {
      ...
      const urlSafeCredentialId = credentialId
        .replace(/\+/g, "-")
        .replace(/\//g, "_")
        .replace(/=/g, "");
      const response = await fetch(
        `https://debug.liquidauth.com/assertion/request/${urlSafeCredentialId}`,
        {
          method: "POST",
      ...
      if (!response.ok) {
        ...
        throw new Error(
          `Failed to get assertion request: ${response.status} ${response.statusText} - ${errorText}`,
        );
      }
      ...
      if (!submitResponse.ok) {
        ...
        throw new Error(
          `Failed to submit assertion response: ${submitResponse.status} ${submitResponse.statusText} - ${errorText}`,
        );
      }
  ...
  const handleCreatePasskey = async () => {
    try {
      ...
      if (!response.ok) {
        const errorText = await response.text();
        throw new Error(
          `Failed to get attestation request: ${response.status} ${response.statusText} - ${errorText}`,
        );
      }
      ...
      if (!submitResponse.ok) {
        const errorText = await submitResponse.text();
        throw new Error(
          `Failed to submit attestation response: ${submitResponse.status} ${submitResponse.statusText} - ${errorText}`,
        );
      }
    ...
    <SafeAreaView style={styles.container}>
      <ScrollView style={styles.container}>
        ...
        <Group name="Functions">
          <View style={{ gap: 10 }}>
            {activePasskeyId && (
              <Text style={styles.credText}>Active Passkey: {activePasskeyId}</Text>
            )}
            {xhdEd25519KeyId && (
